...
映射的目标在配置中以规则为单位进行处理，在运行时中以合并后的匹配结果为单位进行处理：

在配置解析阶段：

- 单个映射目标同时包含 IP 和域名时，只保留第一个域名。
- 单个映射目标包含数个域名时，只保留第一个域名。
- 单个映射目标仅包含 IP 时，使用 IP 列表。

在运行时：

- 匹配结果同时包含 IP 和域名时，删去其中的域名，只返回 IP 地址。
- 匹配结果仅包含数个域名时，存在歧义，查询失败，以空响应终止，不再进入 DNS 查询阶段。
- 匹配结果中有且仅有一个域名时，这个域名将重新进入 Hosts 模块递归解析，重复上面的步骤，最大递归深度为 5。

特别地，若使用`#`开头的 RCode 作为映射目标，则返回对应的响应码。不同代码的含义[参见](https://www.iana.org/assignments/dns-parameters#dns-parameters-6)。
