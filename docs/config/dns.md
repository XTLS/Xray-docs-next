...
- 解析目标地址进行连接：
  - 如在 `freedom` 出站中，将 `domainStrategy` 设置为 `UseIP`，由此出站发出的请求，会先将域名通过内置服务器解析成 IP，然后进行连接。
  - 如在 `sockopt` 中，将 `domainStrategy` 设置为 `UseIP`，此出站发起的系统连接，将先由内置服务器解析为 IP，然后进行连接。
...
其匹配格式（`domian:` `full:` 等等）同常用的 [路由系统](./routing.md#ruleobject) 中的 domain。不同的是，无前缀时此处默认使用 `full:` 前缀（类似常见的 hosts 文件写法）。
