...
允许在其中填入 IPv6 CIDR 块（如 114:514:1919:810::/64），Xray 将会使用地址块中随机的 IP 地址对外发起连接。需要正确配置网络接入方式，路由表以及内核参数以允许 Xray bind 至地址块内的任意 IP。
...
::: tip
填负数（如 `-1`）时，不使用 Mux 模块承载 UDP 流量。将使用代理协议原本的 UDP 传输方式。例如 `Shadowsocks` 会使用原生 UDP，`VLESS` 会使用 UoT。
:::
...
- 默认 `reject` 拒绝流量（一般浏览器会自动回落到 TCP HTTP/2）。
- `allow` 允许走 Mux 连接。
