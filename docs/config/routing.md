...
- `"AsIs"`：保持原样，默认值。路由模块不把域名解析为 IP。匹配规则时，原始请求上下文中有域名时可匹配含域名规则、有 IP 时可匹配含 IP 规则。
- `"IPIfNonMatch"`：一整轮匹配结束后，当没有匹配任何规则时，将域名解析成 IP（A 记录或 AAAA 记录）再次进行匹配，即便请求上下文已包含 IP；
  - 当一个域名有多个 A/AAAA 记录时，会尝试匹配所有的 A/AAAA 记录，直到其中一个与某个规则匹配为止；
  - 解析后的 IP 仅在路由选择时起作用，转发的数据包中依然使用原始域名或 IP；
- `"IPOnDemand"`：在匹配所有规则前，先将域名解析为 IP 以备用于接下来匹配含有 IP 的规则，即便没有 IP 规则或者请求上下文已包含 IP；
  - 当一个域名有多个 A/AAAA 记录时，会尝试匹配所有的 A/AAAA 记录，直到其中一个与某个规则匹配为止；
  - 解析后的 IP 仅在路由选择时起作用，转发的数据包中依然使用原始域名或 IP；
