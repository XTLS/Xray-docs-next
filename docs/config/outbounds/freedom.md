...
- 当使用 `"AsIs"` 时，Xray 将遵循 `streamSettings.sockopt` 中 `domainStrategy` 和 `happyEyeballs` 的设置。若它们也都是默认值，则直接使用 go 自带的 Dial 解析域名并发起连接，优先级固定为 RFC6724 的默认值 (不会遵守 gai.conf 等配置) (人话：IPv6 优先)。出于一些原因，UDP 连接如果使用域名会无视系统设置优先 IPv4。
