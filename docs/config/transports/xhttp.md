...
::: tip
XHTTP 可与 REALITY 组合使用，详见 [REALITY](../transports/reality.md)。
:::
...
服务端若设了 `host`，将会检查客户端发来的值是否一致，否则不会检查。建议没事不设。
...
客户端 padding 通过 `Referer` 头部实现（格式为 `Referer: ...?x_padding=XXX...`），服务端 padding 通过 `X-Padding` 响应头实现。
...
::: tip
上下行分离时下行配置是完全独立的，不会继承上行的任何配置。即使 XMUX 均未填而取默认值时，各自的随机值也是独立无关的，反分析效果更好。
:::
