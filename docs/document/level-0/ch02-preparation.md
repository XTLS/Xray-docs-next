...
1. 在 VPS 的后台安装当前 Debian 稳定版 64bit 系统（本文修订时为 Debian 13 / trixie）
...
::: tip 说明
关于选择 Debian 稳定版作为操作系统，这里稍微多说一句：不管你在网上听说了什么，不管哪个大神告诉你 XXX 版的 Linux 更好、XXX 版的 Linux 更牛，这些 Linux 的派系之争**跟现在的你半毛钱关系也没有**！直接使用当前 Debian 稳定版，已经足以让你的 VPS 服务器在安全、稳健运行的同时得到足够的优化（如 cloud 专用内核、及时的 `BBR` 支持等）。等你对 Linux 熟悉之后，再回头去尝试其他的 Linux 发行版也不迟。
:::
