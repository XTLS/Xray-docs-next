...
本文演示如何使用 Xray 的 VLESS 反向代理能力，通过公网服务器把流量送回内网。这里给出两种常见用法：

- `入口转发`：将公网入口端口映射到内网 Web 服务；
- `远程回家`：用户通过公网服务器中转，回到家里的内网继续访问资源。
...
内网设备的职责是主动连出，并建立反向通道。这里额外写一组路由，是为了把从 `reverse-in` 进入的流量明确送到指定的 `freedom` 出站，而不是完全依赖默认出站，因为通常你的内网 Xray 还会承担日常代理或科学上网功能。

示例里保留了两个 `freedom` 出站：

- 一个普通 `freedom`，作为默认直连出口<br>
  （假设你还有科学上网需求，这里省略了你科学上网的配置）；
...
这种用法的语义很明确：用户感知到的是“访问公网服务器上的 Web 服务”，而实际处理请求的是内网设备上的 Web 服务。
...
- 用于建立反向连接的 UUID，建议不要和普通用户共用；
- 公网服务器务必保留明确的默认出站，避免未命中路由的流量误入反向代理；
- 示例配置偏重说明原理，真实公网环境中通常还需要结合更完整的传输与伪装方案。

## 远程回家

用户通过公网服务器中转，回到家里的内网继续访问资源。
...
家中设备依然要主动连接公网服务器建立反向通道。和第一篇不同，这次它不是把流量统一改写到一个固定 Web 服务，而是把来自 `reverse-in` 的流量统一交给家里的默认直连出口处理。
...
- 第一篇是“把公网一个入口端口映射到内网某个固定服务”；全世界谁都可以访问此服务
