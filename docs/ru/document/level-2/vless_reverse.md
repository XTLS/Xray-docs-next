...
В этой статье показано, как использовать возможность обратного проксирования VLESS в Xray, чтобы через публичный сервер возвращать трафик во внутреннюю сеть. Ниже рассмотрены два распространенных сценария:

- `Проброс входа`: сопоставление публичного входного порта с внутренним Web-сервисом;
- `Удаленно домой`: пользователь проходит через публичный сервер и затем продолжает доступ к ресурсам домашней сети.

## Проброс Входа

Сопоставление публичного входного порта с внутренним Web-сервисом.
...
Задача внутреннего устройства состоит в том, чтобы само инициировать исходящее соединение и создать обратный туннель. Здесь дополнительно задается маршрут, чтобы трафик, приходящий через `reverse-in`, явно отправлялся в указанный outbound `freedom`, а не целиком зависел от outbound по умолчанию, потому что на практике внутренний Xray часто еще обслуживает обычный прокси или обход блокировок.
...
- Один обычный `freedom`, который используется как outbound прямого подключения по умолчанию<br>
  (если у вас также есть сценарий обхода блокировок, соответствующая часть конфигурации здесь опущена);
...
- Они не обязаны называться одинаково, главное, чтобы обе стороны соответствовали одной и той же обратной связи;
...
Смысл такого сценария вполне ясен: для пользователя это выглядит как "доступ к Web-сервису на публичном сервере", но фактически запросы обрабатывает Web-сервис на внутреннем устройстве.
...
- UUID, используемый для установления обратных соединений, желательно не совмещать с UUID обычных пользователей;
- На публичном сервере обязательно должен оставаться явный outbound по умолчанию, чтобы трафик, не совпавший с маршрутами, случайно не попадал в обратный прокси;
...
Пользователь проходит через публичный сервер и возвращается в домашнюю сеть, чтобы продолжать доступ к ресурсам.
...
- Первый вариант - это "сопоставить один публичный входной порт с фиксированным внутренним сервисом"; такой сервис может быть доступен любому пользователю из интернета.
...
- Первый вариант больше похож на проброс во внутреннюю сеть;
- Второй вариант больше похож на удаленный роуминг или легкую распределенную сеть.

### Рекомендации По Безопасности

- UUID для обратного соединения и UUID обычных пользователей обязательно должны быть разделены;
...
- Сопоставление публичного входного порта с фиксированным сервисом во внутренней сети;
