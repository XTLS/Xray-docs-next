...
::: warning
Так называемое "преимущество" модифицированных версий `bbr` **очень недолговечно**. Многие такие скрипты не обновляются годами и даже сегодня могут заменить ваше ядро на уже устаревший релиз. Иными словами, их "апгрейд" часто просто уводит вас с актуального стабильного ядра, которое поддерживается вашим дистрибутивом, на более старое ядро с худшей дальнейшей поддержкой. По сути, это уже скорее **даунгрейд** и **ухудшение**.
:::
...
6.  Я так подробно остановился на `BBR`, потому что вокруг него слишком много мифов и откровенной дезинформации, нацеленной на новичков. Надеюсь, теперь у вас есть чёткое представление о том, что такое `BBR` и как он работает. А теперь давайте обновим систему до актуального ядра, поддерживаемого стабильной веткой Debian, и включим `BBR`! (Это действительно просто)

7.  Сначала обновляем индекс пакетов и всю систему, чтобы перейти на актуальное состояние текущей стабильной версии Debian. На момент этой правки это Debian 13 / trixie.

    ```shell
    sudo apt update && sudo apt full-upgrade
    ```

8.  Для большинства VPS на `amd64` достаточно установить или проверить наличие официального метапакета обычного ядра. Он и дальше будет получать обновления ядра из стабильной ветки Debian.

    ```shell
    sudo apt install linux-image-amd64
    ```

9.  Если ваш VPS точно поддерживает его, можно вместо этого использовать **специальное ядро для облачных серверов** `linux-image-cloud-amd64`.
    ...
    ::: warning
    `linux-image-amd64` и `linux-image-cloud-amd64` — это два альтернативных варианта. Нужен только один из них. Второй вариант компактнее и экономнее по ресурсам, но некоторые пользователи сталкивались с тем, что на неподдерживаемых системах его принудительная установка приводила к невозможности загрузки.

    Чтобы не попасть в такую ситуацию:
    - создайте снапшот системы перед установкой или
    - убедитесь, что у вас есть доступ к `vnc`-консоли (и вы знаете, как ей пользоваться)

    :::

10. Создаём отдельный файл конфигурации `sysctl` и включаем `BBR`:
    ...
    ::: warning
    По сравнению с прямым редактированием `/etc/sysctl.conf`, размещение пользовательских параметров в `/etc/sysctl.d/` лучше соответствует современным практикам Debian / `systemd` и снижает риск того, что эти настройки будут случайно перезаписаны позже. Если у вас уже есть своя схема именования, можете продолжать использовать её.
    :::

11. Добавляем следующие строки в файл:
...
12. Сразу загружаем новые параметры `kernel`:

    ```shell
    sudo sysctl --system
    ```

13. Перезагружаем VPS. Если выше вы устанавливали новое ядро, именно после этого оно тоже начнёт использоваться:

    ```shell
    sudo reboot
    ```

14. Весь процесс показан на гифке:

    ::: tip
    Эта гифка была записана для более старой версии руководства, поэтому показанные в ней имена пакетов и команды могут не полностью совпадать с актуальным текстом выше. Ориентируйтесь прежде всего на текст. Если вы не уверены, поддерживается ли cloud-ядро на вашем VPS, выбирайте обычное ядро `linux-image-amd64`.
    :::

    ![Обновление ядра Debian и включение `BBR`](./ch07-img06-bbr-proper.gif)

15. Проверяем, что `BBR` включен

    Чтобы убедиться, что `BBR` активен, выполните команду:
    ...
    Чтобы убедиться, что используется алгоритм `fq`, выполните команду:
