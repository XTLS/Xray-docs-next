...
Используется для настройки параметров QUIC в XHTTP H3, Hysteria и MASQUE.

> `congestion`: reno | bbr | brutal | force-brutal

Алгоритм управления перегрузкой. В Hysteria по умолчанию используется `brutal`, а в XHTTP H3 и MASQUE - `bbr`.
...
Обратите внимание: XHTTP H3 и MASQUE не могут использовать режим `brutal`, потому что у них нет механизма согласования (MASQUE работает с `bbr`), но поддерживают `force-brutal`, которому согласование не требуется.
