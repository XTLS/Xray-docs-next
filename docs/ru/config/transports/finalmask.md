...
`listenIp`: IP-адрес, на котором выполняется прослушивание. По умолчанию `"0.0.0.0"`.

Обратите внимание, что это отличается от обычного TCP/UDP-прослушивания на `"0.0.0.0"` и `"::"`. Поскольку ICMP на основе IPv4 и ICMPv6 на основе IPv6 не являются взаимозаменяемыми протоколами, указание `"0.0.0.0"` здесь означает прослушивание только ICMP поверх IPv4, и наоборот.
