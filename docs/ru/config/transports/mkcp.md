...
Также была удалена стандартная обфускация mKCP; для подключения к старым версиям серверов необходимо настроить `mkcp-legacy` в FinalMask (если оставить `settings.header` и `settings.value` пустыми, будет использоваться стандартная обфускация XOR из старых версий).
:::
...
Множитель окна перегрузки, применяемый к количеству отправляемых пакетов «в полёте», которое вычисляется из `uplinkCapacity`, `mtu` и `tti`. Минимальное значение — `1`.

По умолчанию `1`.

> `maxSendingWindow`: number

Максимальное окно отправки в байтах. Фактически пересчитывается в количество пакетов «в полёте» как `maxSendingWindow / mtu`, поэтому не должно быть меньше `mtu`.

По умолчанию `2097152`.
