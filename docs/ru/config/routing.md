...
- `random`: значение по умолчанию. Случайным образом выбирает соответствующий исходящий прокси. Если задан `fallbackTag` и настроен [observatory](./observatory.md#observatoryobject) или [burstObservatory](./observatory.md#burstobservatoryobject), перед выбором исключаются прокси, не прошедшие проверку. Если `fallbackTag` не задан или не настроен observatory, выбор выполняется среди всех соответствующих исходящих прокси, включая потенциально недоступные.
- `roundRobin`: выбирает соответствующие исходящие прокси по очереди. Если задан `fallbackTag` и настроен [observatory](./observatory.md#observatoryobject) или [burstObservatory](./observatory.md#burstobservatoryobject), перед выбором исключаются прокси, не прошедшие проверку. Если `fallbackTag` не задан или не настроен observatory, выбор выполняется среди всех соответствующих исходящих прокси, включая потенциально недоступные.
