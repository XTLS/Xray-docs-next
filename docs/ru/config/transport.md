...
В предыдущих версиях, когда Xray пытался использовать доменное имя для установления системного соединения, его разрешение выполнялось системой и не контролировалось Xray.  
Это приводило к таким проблемам, как [невозможность разрешить доменное имя в нестандартных Linux-средах](https://github.com/v2ray/v2ray-core/issues/1909).  
Для решения этой проблемы в Xray 1.3.1 в Sockopt был добавлен параметр `domainStrategy` в разделе Freedom.
