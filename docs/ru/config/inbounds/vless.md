...
Большинству пользователей рекомендуется использовать команду `xray vlessenc` для автоматической генерации этого поля, чтобы избежать ошибок. Подробные настройки ниже рекомендуются только для продвинутых пользователей.

::: details Подробная конфигурация
Его формат представляет собой строку подробных настроек, соединенных символом `.`. Например, `mlkem768x25519plus.native.600s.100-111-1111.75-0-111.50-0-3333.ptjHQxBQxTJ9MWr2cd5qWIflBSACHOevTauCQwa_71U`. В этом документе отдельная часть, разделенная точкой, называется блоком.
...
- Второй блок, это внешний вид трафика, возможные варианты: `native`/`xorpub`/`random`, которые соответствуют: пакет данных в исходном формате / исходный формат + обфускация части открытого ключа / полностью случайное число (подобно VMESS/Shadowsocks). Требуется, чтобы сервер и клиент совпадали.
...
Задает режим управления потоком по умолчанию для всех пользователей этого входящего соединения. Допустимые значения те же, что и у `flow` в [UserObject](#userobject).

Если пользователь не указал `flow` (или указал пустую строку), для него применяется значение, заданное здесь. Указанный самим пользователем `flow` имеет более высокий приоритет.

::: warning
Если здесь указано `xtls-rprx-vision`, отключить управление потоком для отдельного пользователя уже невозможно.
:::
...
- Отсутствует `flow` или пустая строка: применяется значение `settings.flow` входящего соединения; если `settings.flow` также не задан, используется обычный TLS-прокси
